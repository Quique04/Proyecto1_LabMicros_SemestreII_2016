...
%macro Sumar_Barra_Aux 2
        cmp %2, 52      ;52 => ascii decimal para '4'; compara unidades(posicion vs limite);%2 = pos_barra_uni
        jz comp_dec_lim_aux
        jnz comp_uni_tope_aux       ;tope => 9

comp_dec_lim_aux:
        cmp %1, 51              ;51 => ascii decimal para '3'; compara decenas(posicion vs limite);%1 = pos_barra_dec
        jz final_macro_suma_aux ;llego al tope, no hace nada
        jnz suma_1_uni_aux

comp_uni_tope_aux:
        cmp %2, 57              ;57 =>ascii decimal para '9'; compara unidades(posiciones vs tope)
        jz suma_1_dec_aux           ;suma 1 en las decenas(%1) y mueve un '0' (48) a las unidades (%2)
        jnz suma_1_uni_aux

suma_1_uni_aux:
        inc %2
        jmp final_macro_suma_aux
...
final_macro_suma_aux:
%endmacro

;-------------------------------------------------------------------------------------

%macro Restar_Barra_Aux 2
        cmp %1, 48              ;48 => ascii decimal para '0'; compara decenas(posicion vs tope);%1 = pos_barra_dec
        jz comp_uni_tope_resta_aux       ;pregunta si %1 es igual a '0'
        jnz resta_1_uni_aux         ;resta 1 a %2;%2 = pos_barra_dec

comp_uni_tope_resta_aux:
        cmp %2, 56      ;56 => ascii decimal para '8'; compara decenas(posicion vs tope);%2 = pos_barra_uni
        jz final_macro_resta_aux
        jnz resta_1_uni_aux
...
perdio: db 27,'[30;47m',0x1b,"[20;3f","Perdio",0x1b,"[21;3f"," presione Enter para continuar"
Lperdio: equ $-perdio

gano:db  0x1b,"[20;3f", 0x1b,"[49;39m"," FELICIADES HA GANADO!!!!"
ganoL equ $-gano

perdioV: db 0x1b,"[20;3f","Intentelo de nuevo",0x1b,"[21;3f"," presione Enter para continuar"
LV: equ $-perdioV
...
pos_barra_dec_aux: resb 1
pos_barra_uni_aux: resb 1
...
tecla_c:
        ...
        Sumar_Barra_Aux byte [pos_barra_dec_aux], byte [pos_barra_uni_aux]
        escribe borrar_barra, borrar_barra_tam
        jmp Continue

tecla_z:
        Restar_Barra byte [pos_barra_dec], byte [pos_barra_uni]
        Restar_Barra_Aux byte [pos_barra_dec_aux], byte [pos_barra_uni_aux]
        escribe borrar_barra, borrar_barra_tam
        jmp Continue
...
;-----una ves en la posicion deseada imprime la bola 
        ...
        mov rcx, 99999999                       ; la velocidad de la bola en el juego
...
cmpBolaBloque1:
    ...
    X_U:
        ...
        WhereBarra:
                ...
                push r8
                push r15                
                mov r8,[pos_barra_dec]
                movzx r15, byte [pos_barra_dec_aux]
                pare_revise_decenas:
                cmp r8,r15
                je VeaUnid
                jne WhereBarra2
        ...
        VeaUnidAux:
                movzx r15, byte [pos_barra_uni_aux]
                pare2:
...
mov byte [pos_barra_dec_aux], 49

mov byte [pos_barra_uni_aux], 55
